...
# on every master reboot or a single time on cluster deployment,
# respectively.
#
# Examples:
#   Submit a single job with cluster and turn off cluster on completion:
#
#     gcloud dataproc clusters create ${CLUSTER_NAME} --scopes cloud-platform --initialization-actions gs://${BUCKET}/dataproc-master-onboot.sh --metadata onboot-command="gcloud dataproc jobs submit hadoop --cluster ${CLUSTER_NAME} --jar file:///usr/lib/hadoop-mapreduce/hadoop-mapreduce-examples.jar teragen 10000 /tmp/teragen; gcloud dataproc clusters delete -q ${CLUSTER_NAME}"
#
#   Create a cluster which runs a "forever-running" job, resubmitting on reboot
#   or if the job terminates for any reason:
#
#      gcloud dataproc clusters create ${CLUSTER_NAME} --scopes cloud-platform --metadata startup-script-url=gs://${BUCKET}/dataproc-master-onboot.sh,onboot-command="while true; do gcloud dataproc jobs submit spark --cluster ${CLUSTER_NAME} --jar gs://${BUCKET}/my-longlived-job.jar foo args; done"
...
# The command is specified with the 'onboot-command' key.
ONBOOT_COMMAND=$(curl -f -s -H Metadata-Flavor:Google \
    ${METADATA_ROOT}/onboot-command)

if [ -z ${ONBOOT_COMMAND} ]; then
  echo "Failed to find metadata key 'onboot-command'"
  exit 1
fi
...
nohup /usr/local/bin/await_cluster_and_run_command.sh &>> \
    /var/log/dataproc-master-onboot.log &
